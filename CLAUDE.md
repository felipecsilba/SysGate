...
- **Chamados — Pegar/Delegar para mim**: botão no cabeçalho do painel de detalhe (abaixo dos badges). Sem responsável → verde "Pegar para mim" (delega direto). Com outro responsável → âmbar "Delegar para mim" (abre `ConfirmDialog`). Não exibe se já é do usuário logado ou chamado encerrado. Usa `atualizarCampo('responsavelId', usuario.id)` — badge contSemDono atualiza automaticamente.
